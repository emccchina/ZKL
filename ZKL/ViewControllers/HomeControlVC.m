...
- (void)requestForPlanModles:(PlanModel*)planModel
{
    ...
    NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:planModel.title, @"title",titleHour, @"totalHour",planModel.beginDate,@"beginString",planModel.endDate, @"endString",[UserInfo shareUserInfo].userCode, @"userCode", planModel.unableMinute, @"unableMinute",nil];
    NSLog(@"dict %@", dict);
...
- (void)requestForEditDream:(PlanModel*)planModel
{
    ...
    NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:planModel.title, @"title", total, @"totalHour",planModel.beginDate,@"beginString",planModel.endDate, @"endString",[UserInfo shareUserInfo].userCode, @"userCode",planModel.planIDServer,@"planCode", planModel.unableMinute, @"unableMinute",nil];
    NSLog(@"dict %@", dict);
    [manager POST:url parameters:dict success:^(AFHTTPRequestOperation *operation, id responseObject) {
        NSLog(@"responseObject is %@", [[NSString alloc] initWithData:responseObject encoding:NSUTF8StringEncoding]);
